...
indexDefault (_ : xs) n def = indexDefault xs (n - 1) def
...
mymaximum comp initial (x : xs) = mymaximum comp (if comp initial x then initial else x) xs
