...
rangeOf :: (Num a, Foldable t, Ord a) => t a -> a
...
longest :: (Ord a) => [[a]] -> [a]
longest = maximumBy comp
  where
    comp xs ys = case comparing length xs ys of
        EQ -> if head xs < head ys then GT else LT
        res -> res
...
incrementKey k = map (\x@(a, b) -> if a == k then (a, b + 1) else x)
...
winner scores player1 player2
    | getScore player1 >= getScore player2 = player1
    | otherwise = player2
  where
    getScore k = Map.findWithDefault 0 k scores
...
transfer from to amount bank = case (Map.lookup from bank, Map.lookup to bank) of
    (Just a1, Just a2)
        | amount >= 0 && a1 >= amount ->
            Map.adjust (subtract amount) from . Map.adjust (+ amount) to $ bank
    _ -> bank
