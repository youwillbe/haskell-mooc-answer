...
combine z (Picture f1) (Picture f2) = Picture f
  where
    f c = z (f1 c) (f2 c)
...
paintSolid color (Shape fs) (Picture fb) = Picture f
  where
    f c
        | fs c = color
        | otherwise = fb c
...
instance Transform Blur where
    apply Blur (Picture f) = Picture g
      where
        g = blur . map f . neighbours
        neighbours c@(Coord x y) =
            [ c
            , Coord (x + 1) y
            , Coord (x - 1) y
            , Coord x (y - 1)
        ...
        blur cs =
            Color
                ((sum . map getRed $ cs) `div` 5)
                ((sum . map getGreen $ cs) `div` 5)
                ((sum . map getBlue $ cs) `div` 5)
