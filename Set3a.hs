...
firstHalf s = take n s
  where
    l = length s
    n = if odd l then div l 2 + 1 else div l 2
...
while check update value = go value
  where
    go v
        | check v = go $ update v
        | otherwise = v
...
joinToLength n xs = [s | x <- xs, y <- xs, let s = x <> y, length s == n]
...
interpreter s = go s (0, 0)
  where
    go [] _ = []
    go ("printX" : ss) t@(x, _) = show x : go ss t
    go ("printY" : ss) t@(_, y) = show y : go ss t
    go ("up" : ss) (x, y) = go ss (x, y + 1)
    go ("down" : ss) (x, y) = go ss (x, y - 1)
    go ("left" : ss) (x, y) = go ss (x - 1, y)
    go ("right" : ss) (x, y) = go ss (x + 1, y)